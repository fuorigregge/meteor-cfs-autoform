Hooks = {
    ...
    beforeMethodUpdate: function(modifier, id) {
        ...
        template.$('.cfsaf-hidden').each(function() {
            ...
            // no matter what, we want to delete the dummyId value
            //delete doc[key];
            CfsAutoForm.Util.deepDelete(modifier.$set, key);
    ...
    beforeUpdate: function(doc) {
        ...
        // We pass back the doc synchronously
        if (totalFiles === 0) {
            //se non ci sono file e c'è il vecchio id, rimuovi immagine
            var oldId = template.$('.cfsaf-hidden').attr('ref');
            if (oldId) {
                if (! doc.$unset) doc.$unset = {};
                doc.$unset.imageId = '';
                if (doc.$set && doc.$set.imageId) {
                    delete doc.$set.imageId;
                }
            }
            return doc;
        }
    ...
    afterUpdate: function(error, result) {
        ...
        if (error) {
            ...
            // cleanup files data
            var key = elems.attr("data-schema-key");
            var oldId = elems.attr('ref');
            console.log(oldId, result.imageId, key, elems.length);
            //if(textField.attr('ref') != result.imageId && elems.length){
            //se compare l'elemento file-hidden in un form di update cancella la vecchia immagine in after update
            //non usare ref?!
            if (oldId != result.imageId && elems.length) {
                console.log("remove image");
                var fsCollectionName = elems.attr("data-cfs-collection");
                var fsCollection = FS._collections[ fsCollectionName ];
                fsCollection.remove(oldId, function(e, r) {
                    console.log(e, r);
                });
            }
